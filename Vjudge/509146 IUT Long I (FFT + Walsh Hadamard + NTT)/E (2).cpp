...
{
    long double x, y;
    CD(long double x=0, long double y=0) :x(x), y(y) {}
    CD operator+(const CD& o) { return {x+o.x, y+o.y};}
    CD operator-(const CD& o) { return {x-o.x, y-o.y};}
    CD operator*(const CD& o) { return {x*o.x-y*o.y, x*o.y+o.x*y};}
    void operator /= (long double d) { x/=d; y/=d;}
    long double real() {return x;}
    long double imag() {return y;}
};
...
vector<bool> bigmod(vector<bool> &v, LL k)
{
    vector<bool> ret(1);
    ret[0] = 1;
