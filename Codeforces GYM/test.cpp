...
//int tree[100005];int query(int idx){int sum=0;for(;idx>0;idx-=idx & (-idx)) sum+=tree[idx];return sum;}void update(int idx,int val,int n){for(;idx<=n;idx+=idx & (-idx)) tree[idx]+=val;}
...
//struct point{LL x,y;bool operator < (const point &p) const{return x<p.x || (x==p.x && y<p.y);}} P[MAX],C[MAX];inline LL Cross(point &o,point &a,point &b){return (a.x-o.x)*(b.y-o.y)-(a.y-o.y)*(b.x-o.x);}void ConvexHull(int np,int &nc){sort(P,P+np);REP(i,np){while(nc>=2 and Cross(C[nc-2],C[nc-1],P[i])<=0)nc--;C[nc++]=P[i];}int t=nc+1;ROF(i,np-1,1){while(nc>=t and Cross(C[nc-2],C[nc-1],P[i-1])<=0)nc--;C[nc++]=P[i-1];}nc--;return;}void init(){int nc=0,np;scanf("%d",&np);REP(i,np){scanf("%lld %lld",&P[i].x,&P[i].y);}ConvexHull(np,nc);REP(i,nc){printf("%lld %lld\n",C[i].x,C[i].y);}}
...
{
    if(val==0 and pos==0) return 1;
    else if(val==0) return 0;
    if(dp[pos][val]!=-1) return dp[pos][val];
    LL ans=0;
    if(pos+1<=k) ans+=call(pos+1,val-1);
    ans%=MOD;
    if(pos-1>=0) ans+=call(pos-1,val-1);
    return dp[pos][val]=ans%MOD;
}
...
int main()
{
    ...
    MEM(dp,-1);
    LL sum=0;
    for(int i=2;i<=m;i+=2) sum=(sum+call(0,i))%MOD;
    cout << (2LL*n*sum)%MOD;
}
